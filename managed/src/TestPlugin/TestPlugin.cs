...
public class TestPlugin : BasePlugin
{
    ...
    public void RefactoredMenuTestCommand( ICommandContext context )
    {
        ...
        var menu = Core.MenusAPI
            ...
            .AddOption(new TextMenuOption($"<b>{HtmlGradient.GenerateGradientText("Swiftlys2 向这广袤世界致以温柔问候", "#FFE4E1", "#FFC0CB", "#FF69B4")}</b>", MenuOptionTextStyle.ScrollLeftLoop))
            .AddOption(new TextMenuOption($"<b>{HtmlGradient.GenerateGradientText("Swiftlys2 からこの広大なる世界へ温かい挨拶を", "#FFE5CC", "#FFAB91", "#FF7043")}</b>", MenuOptionTextStyle.ScrollRightLoop))
            .AddOption(new TextMenuOption($"<b>{HtmlGradient.GenerateGradientText("Swiftlys2 가 이 넓은 세상에 따뜻한 인사를 전합니다", "#E6E6FA", "#00FFFF", "#FF1493")}</b>", MenuOptionTextStyle.ScrollLeftFade))
            .AddOption(new TextMenuOption($"<b>{HtmlGradient.GenerateGradientText("Swiftlys2 приветствует этот прекрасный мир", "#AFEEEE", "#7FFFD4", "#40E0D0")}</b>", MenuOptionTextStyle.ScrollRightFade))
            .AddOption(new TextMenuOption("<font color='#F5FFFA'><b><garbage><font color='#00FA9A'>Swiftlys2</font> salută această lume minunată</garbage></b></font>", MenuOptionTextStyle.TruncateEnd))
            .AddOption(new TextMenuOption("<font color='#00FA9A'><b><garbage><font color='#F5FFFA'>Swiftlys2</font> extends warmest greetings to this wondrous world</garbage></b></font>", MenuOptionTextStyle.TruncateBothEnds))
